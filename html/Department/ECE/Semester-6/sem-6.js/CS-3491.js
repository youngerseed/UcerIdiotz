...
let unit_text1 = `Introduction to AI - AI Applications - Problem solving agents - search algorithms - uninformed search strategies - Heuristic search strategies - Local search and optimization problems -
adversarial search - constraint satisfaction problems (CSP)`;

let unit_text2 = `Acting under uncertainty - Bayesian inference - naïve bayes models. Probabilistic reasoning -
Bayesian networks - exact inference in BN - approximate inference in BN - causal networks.`;

let unit_text3 = `Introduction to machine learning - Linear Regression Models Least squares - Linear Regression Models single & multiple variables - Bayesian linear regression - gradient descent - Linear Classification Models Discriminant function - Probabilistic discriminative model - Logistic regression - Probabilistic generative model - Naive Bayes - Maximum margin classifier - Support vector machine - Decision Tree - Random forests`;

let unit_text4 = `Combining multiple learners Model combination schemes - Combining multiple learners Voting - Combining multiple learners Ensemble Learning - Combining multiple learners bagging - Combining multiple learners boosting - Combining multiple learners stacking - Unsupervised learning K-means - Instance Based Learning KNN - Gaussian mixture models and Expectation maximization`;

let unit_text5 = `Perceptron - Multilayer perceptron - activation functions - network training - gradient descent optimization - stochastic gradient descent - error backpropagation - from shallow networks to deep
networks - Unit saturation - ReLU - hyperparameter tuning - batch normalization - regularization - dropout.`;
